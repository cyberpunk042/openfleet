...
def ensure_agent_cron_jobs() -> int:
    """Ensure all agents in the gateway config have heartbeat cron jobs.

    Reads ~/.openclaw/openclaw.json for agents with heartbeat.every config.
    Reads ~/.openclaw/cron/jobs.json for existing cron jobs.
    Creates missing cron jobs via 'openclaw cron add' for agents that
    have a heartbeat config but no cron job.

    Returns number of cron jobs created.
    """
    import json as _json
    import subprocess as _sp
    from pathlib import Path as _Path
    ...
    # Read agent list
    oc_config = _Path.home() / ".openclaw" / "openclaw.json"
    ...
    if cron_path.exists():
        try:
            ...
            for job in cron_data.get("jobs", []):
                existing_agent_ids.add(job.get("agentId", ""))
        except Exception:
            pass
    ...
    for agent in agents_with_heartbeat:
        if agent["id"] in existing_agent_ids:
            continue
        # Use openclaw cron add to create the job properly
        try:
            result = _sp.run(
                ["openclaw", "cron", "add",
                 "--agent", agent["id"],
            ...
            if result.returncode == 0:
                created += 1
                logger.info("Created cron job for %s", agent["name"])
            else:
                logger.warning("Failed to create cron for %s: %s",
                             agent["name"], result.stderr[:100])
        except Exception as e:
            logger.warning("Failed to create cron for %s: %s", agent["name"], e)
